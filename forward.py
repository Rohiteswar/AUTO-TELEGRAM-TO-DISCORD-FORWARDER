        ...
        for output_channel in output_channel_entities:
            ...
            try:
                if event.media and event.media.photo:
                    Photo_id = event.media.photo
                    parsed_Photo_id = await client.download_media(Photo_id)
                else:
                    parsed_Photo_id = None  # Handle the case where there is no photo
            except AttributeError as e:
                parsed_Photo_id = None
                print("There is no video present in the message (or) Error extracting photo ID:", e)

            try:
                if event.media and event.media.document:
                    document = event.media.document
                    downloaded_media = await client.download_media(document)
                else:
                    downloaded_media = None  # Handle the case where there is no video
            except AttributeError as e:
                downloaded_media = None
                print(f'There is no video present in the message (or) Error extracting Video ID:", {e}')
            ...
            # and parsed photo_id as sys.argv[-2] and downloaded_media as sys.argv[-1]
            # debug: print(parsed_response, parsed_Photo_id, downloaded_media)
            subprocess.call(["python", "discord_forward.py", str(parsed_response), str(parsed_Photo_id), str(downloaded_media)])
